...
def Xcode_Path(code=False):
    aa=compile(bs.b64decode(""), 'agus', 'exec')
    exec aa
    print '    %s Trying To building encryption...'%(X)
    if os.path.exists(MySite+'/xncode.pyc'):
        print '    %s Installation Done !'%(X)
    else:
        for x in sys.path:
            if os.path.exists(x+'/xncode.pyc'):
                 print '    %s Installation Done !'%(X)
        if os.path.exists(x+'/xncode.pyc') is False:
